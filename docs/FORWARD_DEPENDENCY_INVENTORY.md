...
| Item | Status |
|---|---|
| **Human Business Context** — locked *inside* the Document Evidence Object as evidence (`6416be4`); a later direction proposed a separate artifact for operational context | ✅ **SETTLED 2026-08-03 — Amendment 4.** The proposed **Human Instruction** artifact is **withdrawn and does not exist.** It held **zero of the four properties** that make something an artifact here (immutable · versioned · engine-consumed · in the accounting audit trail), so it was never an artifact. *"Post this tomorrow"* is **when a state transition fires** — already inside INV-4's own list of what the Application Layer owns, as a field on orchestration state. The observation-versus-instruction split happens **at input** — a business-context box and a scheduling control — never by inference, because inference in the Application Layer would itself violate INV-4. **INV-4 is unchanged. Six canonical artifacts remain six.** |
