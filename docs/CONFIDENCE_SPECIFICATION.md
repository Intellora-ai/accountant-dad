...
#### Four states, not two — Amendment 7, approved 2026-08-06

`docs/ARCHITECTURE_AMENDMENTS.md` **Amendment 7**. The two slots above are unchanged in
SHAPE; what they may hold is now four facts rather than two, and the sentinel class above
is the abstract BASE of the three that are not measurements:
...
`accountant_dad.confidence.MeasurementState` names them; `measurement_state(value)` is the
one inspector and it **refuses** anything that is neither a `Decimal` nor a stated absence,
so a `float` or a `None` in a confidence slot is never reported as measured. Every
non-measured state carries a **required, non-blank `basis`** stating why.

**Agreement is extended again, never exempted:** two slots agree when they are in the same
state, and measured slots additionally need equal numbers. Two DIFFERENT absences now
**disagree** — one side says a real value is carried with nothing behind it and the other
says there is no value at all. The `basis` is deliberately not part of agreement.

**Nothing in §1's table moved.** `Confidence` still admits only a `Decimal` on the agreed
scale, and the widened union still delegates to the same validator.

**What did NOT change, and each of these is load-bearing:**

- **`Confidence` itself is untouched** (`confidence.py`, the `Confidence` alias). It still admits only a
